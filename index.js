...
function calculateHypotenuse(a, b) {
  ...
  const heptotenuse = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
...
function roundObjectValues(obj) {
  ...
  const newArr = myArr.map(([key, value])  => {
...
return null;
  ...
  const interest = principal * Math.pow((1 + interestRate), years)
  // Округляємо ії до найближчого цілого
  const a = Math.floor(interest)
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const totalPrice = products.reduce((accum, {title, price}) => accum + price, 0)
  // Додаємо ціну продукту до аккумулятора.
  // Конвертуємо totalPrice та maxPrice за допомогою Math.fround.
  const a = Math.fround(totalPrice);
  const b = Math.fround(maxPrice)  
